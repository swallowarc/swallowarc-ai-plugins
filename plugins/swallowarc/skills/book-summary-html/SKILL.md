...
**詳しさは2モードから選ぶ。** `標準`（既定の凝縮版・現行どおり）と `詳細`（各論点について著者の論拠・前提・筋道まで展開する深掘り版）。引数 `mode=標準` / `mode=詳細`（英語別名 `mode=standard` / `mode=detailed`）で指定でき、指定がなければ生成前に必ずどちらで作るかを確認する。詳細モードでも、本書外の背景は増やさず（専門用語の短い定義のみ例外）、深さの源は出典で裏が取れる著者の論拠に限る。
...
3. **モードを確定してから進む。** 引数に `mode=` 指定がなければ、フェーズ0冒頭で標準／詳細のどちらで作るかを確認し、確定するまで分析・生成へ進まない。

```dot
digraph gate {
  "依頼を受ける" [shape=box];
  "mode引数あり?" [shape=diamond];
  "標準/詳細をユーザに確認" [shape=box];
  ...
  "依頼を受ける" -> "mode引数あり?";
  "mode引数あり?" -> "標準/詳細をユーザに確認" [label="no"];
  "mode引数あり?" -> "引数で本文ファイルあり?" [label="yes"];
  "標準/詳細をユーザに確認" -> "引数で本文ファイルあり?";
...
- 詳しさのモード（標準／詳細）を引数で、または対話で選んで要約サイトを作りたいとき

## Process

### フェーズ0：前提（ソースと書誌情報の確定）

**最初にモードを確定する。** 引数に `mode=標準`／`mode=詳細`（英語別名 `mode=standard`／`mode=detailed`）があればそれに従う。なければ、ソース確定の前に「標準モード（凝縮）と詳細モード（著者の論拠・前提・筋道まで展開）のどちらで作るか」をユーザに確認し、確定してから先へ進む。以降のフェーズはこのモードに従う。
...
提示時は**選択中のモード（標準／詳細）も明記**し、合意対象に含める。詳細モードのときは上記3に加え、各章の主張に**論拠・前提・筋道**をひも付けたリスト（または代表例＋方針）も提示する（`references/analysis-guide.md`）。

### フェーズ2：生成

- `assets/style.css` を出力フォルダにコピーし、各 HTML から相対リンクする。
- index は `templates/index.html`、本文ページは `templates/page.html` を雛形にする。
- **index 冒頭の固定文（必須・文言変更禁止）**：`templates/index.html` の `.about-notice`（「ご利用前の注意」・黄色基調の警告バナー）を `<main>` の先頭に必ず残す。「AIによる本書の Web リサーチ結果から生成したもの」「書籍本文から作成しておらず内容が書籍と一致しない可能性がある」「本の内容を網羅していない」「読む前のイメージづけ／読了後の見直しの参考資料として使う」旨を明記した固定文で、書名等に合わせて書き換えない。
- **index の構成（推奨順）**：⓪ ご利用前の注意（固定文）→ ① 書誌情報 → ② 本書の全体像（著者の主張の骨格・本に基づく）→ ③ この本がターゲットとする読者（本の内容から推定。`.persona-grid`、末尾に「（本書の内容から推定）」と添える）→ ④ おすすめの読み進め方（**AIによる独自分析**。`.ai-analysis` ＋ AIバッジで明示）→ ⑤ 各部/章へのカードナビ。④は本にない情報を書いてよい**唯一の例外**（次項）。
- 各ページは「**要点（見出し）＋解説**」を基本単位とし、グラフィカル部品（`references/style-components.md`）で視認性を上げる。**モードで厚みを変える**：
  - **標準**：`サマリ（簡潔な解説）＋箇条書き`（現行どおり）。
  - **詳細**：`サマリ ＋ 論拠展開段落（著者の論拠・前提・筋道）＋ 箇条書き ＋（必要なら .callout.why）`。見出し・トピックの骨格は標準と同じに保ち、各論点を縦に厚くする。新規の見た目部品は追加しない（既存部品の組み合わせで深さを表現する）。
...
生成後、**独立したレビュアーのサブエージェント**を起動し、`references/review-checklist.md` に照らして検査させる。Agent ツールで「このスキルの review-checklist.md に従い、指定 HTML 群を検査して違反箇所を file:line 付きで列挙せよ」と依頼する。レビュアーは生成器とは別コンテキストで、忖度せず指摘する役割を負う。指摘を本文へ反映し、必要なら再レビューする。レビュー依頼時は**対象モード（標準／詳細）を伝え**、詳細モードのときはチェックリストの「G. 詳細モード固有」も適用させる。

## Quick Reference

| 項目 | 規定 |
|---|---|
| 詳しさモード | `標準`（凝縮）／`詳細`（論拠・前提・筋道まで展開）。引数 `mode=` 指定、なければ生成前に確認。標準は現行と同等 |
| 文体 | だ・である調に統一。用語も統一（表記ゆれ禁止） |
| 本にない情報 | 原則書かない。補う場合は `補足（本書外）` 部品で明示し、誤りでないことを確認 |
| 禁止 | 比喩・情緒・感情・ポエム・著者にない主張／つなぎの空文 |
| index | ご利用前の注意(固定文) → 書誌情報 → 全体像 → ターゲット読者 → 読み進め方(AI) → 各部カード の順 |
| 冒頭固定文 | `.about-notice`（AI生成・書籍と不一致の可能性・非網羅・読書サポート用途）を index 冒頭に必ず残す。黄色基調の警告バナー。文言変更禁止 |
...
- **モード未確定で生成を始める** → 引数に `mode=` がなければフェーズ0冒頭で標準／詳細を確認する。
- **詳細モードで量を緩めて水増しする** → 詳細は「書いてよい情報の種類（論拠・前提・筋道）」を増やすだけ。つなぎの空文・言い換えの禁止は不変。

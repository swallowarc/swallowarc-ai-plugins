...
article_type=="news") で「参考情報が必須か」を判定する。genko には
...
None」という、genko の 2 引数では表現不能な組み合わせ（下記
...
TestCheckReferences_AllPass
    -> test_check_references_all_pass
       ...
       required=True にするが、genko の required シグナルは research_content
...
1. TestCheckReferences_SectionMissing の "required by research profile" サブケース:
   ...
   まだ無い状態）。genko の check_references は generation_profile を
...
2. TestCheck_IncludesReferenceChecks:
   ...
   統合テスト（本番のチェック登録・実行順序の検証）。genko 側の対応物は
...
test_check_references_required_by_research_content_present_section_missing:
   上記の除外 1 で失われる「research 由来で必須」の観点を、genko の
...
def test_check_references_all_pass():
    ...
    # するが、genko の required シグナルは research_content の有無のため、

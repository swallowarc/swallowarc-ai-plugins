...
リファクタリング（挙動不変の変更）に必要なのは新しい失敗テストではなく、既存テストの green 維持である（REFACTOR フェーズと同じ扱い）。失敗するはずのないテストをわざわざ作って偽の RED を演出しない。
...
既存テストへの波及は、挙動ごとに全体を回して確かめるのではなく、反復中は変更箇所に関係するテストを回し、コミット前に一度だけ全体を実行して確認する。dispatch 元からテスト範囲の指定を受けているサブエージェントは、その指定に従う。
...
テストを後から書いても、書いた瞬間に通ってしまう。実装のどの分岐も一度も失敗を経由していないので、そのテストが本当にバグを検出できるのかは誰にも分からないままである。さらに後付けのテストは実装に引きずられる。すでに書いたコードを見ながら書くため、「要求は何か」ではなく「このコードは何をしているか」を検証してしまい、書き忘れたエッジケースは後からテストを書いても同じように書き忘れる。

手動確認とサンクコストが省略の理由にならないことは、「合理化への反論」の表が扱う。

...
選択肢を出す前に、プロジェクトのテストが全件通過していることを確かめる。判定基準は `kata:verify-done` の鉄の掟に従う——同一コード状態（同一 HEAD かつクリーンな作業ツリー）へのフルスイートの証跡をこのセッション内でコマンドと出力つきで指せるならそれでよく、指せないならここで実行する。「たぶん通る」で済ませない。
...
選ぶのに必要な情報は、選択肢そのものの中に収める。ダイアログの外に出力したテキストが、ユーザーに読まれない環境があるためである。判断材料をダイアログの前の本文に書いて、選択肢からそれを参照させるような出し方はしない。
...
- 判断材料をダイアログの外の本文に置き、選択肢からそれを参照させる

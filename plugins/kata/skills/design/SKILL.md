...
3. 未確定点を棚卸しして解消する
4. 2〜3 案を比較して提示する
5. 設計をセクションごとに提示し、都度確認を取る
6. 承認された設計を spec として文書化する

コード 1 行も書く前に、この 6 段階を踏む。
...
5. **沈黙** — 書かれるべきなのに書かれていない挙動はないか。棚卸しと同じ観点レンズ（境界値・失敗時の挙動・同時実行・互換性・性能・外部依存の実挙動）で spec を読み直して探す。設計判断を左右しない沈黙はその場で inline 修正する。左右する沈黙は棚卸しの取りこぼしなので、勝手に埋めず、棚卸しの解消手段（ユーザーへの質問・調査）に戻して解消し、検討済み事項に追記する

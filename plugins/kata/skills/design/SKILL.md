...
digraph design_flow {
    ...
    "2〜3案を提示" -> "オープンクエスチョン棚卸し" [label="採用案に固有の未知を追加"];
    "2〜3案を提示" -> "設計をセクション提示";
...
5. **沈黙** — 書かれるべきなのに書かれていない挙動はないか。棚卸しと同じ観点レンズ（境界値・失敗時の挙動・同時実行・互換性・性能・外部依存の実挙動）で spec を読み直して探す。設計判断を左右しない沈黙はその場で inline 修正する。左右する沈黙は棚卸しの取りこぼしなので、勝手に埋めずユーザーへの質問に戻って解消し、検討済み事項に追記する

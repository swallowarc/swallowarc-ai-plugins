...
プランの形式を判別する。タスクに「受け入れ基準」欄があれば**受け入れ基準型**、コード付きステップがあれば**コードステップ型**（従来形式）であり、自走モードの進め方と wave 条件の「実質タスク」判定がこの区別を参照する。コードステップ型は従来どおり記述どおりに実行する（後方互換）。1 つのプラン内での形式混在、および受け入れ基準型プランへの実装本体・テスト本体のコードブロック混入は、プランの欠陥として前段の「一度だけまとめて確認」に含める。

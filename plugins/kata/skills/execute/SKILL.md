...
execute は、書かれたプランを読み、批判的にレビューし、隔離された作業環境で全タスクを実行しきるスキルである。まず Step 0 で作業環境を分離し、次にプランを読み込み、実行モード（自走 / サブエージェント駆動）を選んで宣言し、タスクループを回す。
...
ユーザーが会話の中で実行方式を既に指定している場合（`kata:plan` からの引き継ぎ時の指定を含む）は、無条件にそれに従う。以降の判断・質問は行わない。
...
> 「実行モード: サブエージェント駆動で進めます。タスクが互いに独立しており、タスクごとにレビューを挟む価値があるためです。」

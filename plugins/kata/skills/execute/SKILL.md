...
**モデル選択**: モデルは名前でなく階級（安価な階級／セッション継承／最も能力の高い階級）で扱い、既定は省略——セッションモデルの継承——とする。明示するのは階級を変えたい dispatch だけである。

- **implementer**: 単一ファイルの機械的修正（typo・設定値・ドキュメント）とコードステップ型プランの転記的タスクは、利用可能な選択肢のうち安価な階級を明示する。コードを書くタスク（実装判断を伴う）と複数ファイルの統合は省略する
- **タスクレビュアー**: 省略する。レビューは実装の検証の砦であり、実装側の節約と連動して下げない
- **最終ブランチレビュー**: 最も能力の高い階級を明示する
- **安全弁**: Agent ツールに model パラメータがない・選択肢の序列を判断できない・親が既に最軽量で下げ先がない——いずれの場合も明示せず省略する。迷ったときも省略する。省略は現状維持であり、誤判断しても全 dispatch がセッションモデルで走るだけで、現行より悪くなる経路はない
...
```bash
# 未 ignore なら追加する（子パスで判定する理由は Step 0 参照）
git check-ignore -q .worktrees/probe || { printf '.worktrees/\n' >> .gitignore; git add .gitignore; git commit -m "chore: worktree ディレクトリを ignore する"; }
...
2. **完了主張の検証**: `kata:verify-done` に従って完了を主張する。最終レビュー後に修正を行った場合はフルスイートを再実行する。修正がなければ、直近の統合検証（同一 HEAD へのフルスイート green）の証跡がそのまま証拠になり、回し直しは不要である。

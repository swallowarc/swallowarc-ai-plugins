...
6. ガードを通過したタスクから順に、統合 worktree で `git diff -U10 BASE..<タスクブランチ>` を一意な名前のファイルへ書き出し、reviewer を dispatch する。修正往復もそのタスクの worktree・ブランチ上で行う

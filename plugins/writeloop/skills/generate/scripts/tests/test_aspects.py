...
def test_general_gets_only_readability_and_metaphor():
    ...
    assert got == ["lead_quality", "heading_informativeness", "metaphor_discipline"]
...
def test_document_mode_reduced_set():
    ...
    assert got == ["lead_quality", "heading_informativeness", "metaphor_discipline", "source_fidelity"]
...
# 先頭 3 文字列リテラルの結合）はそのまま一致することを要求し、末尾は writeloop の局所適応

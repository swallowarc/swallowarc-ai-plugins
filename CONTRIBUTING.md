...
genko のスクリプトには pytest のテストがある。
`plugins/genko/**` の変更を push すると CI（`.github/workflows/genko-tests.yml`）が実行する。
ローカルでは次で実行できる。

```
uv run --with pyyaml --with regex --with pytest python -m pytest plugins/genko/skills/generate/scripts/tests -q
```

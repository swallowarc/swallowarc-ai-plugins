...
- **`swallowarc`** — 個人用の汎用スキル集。`/swallowarc:<skill-name>` で呼び出す。新しい単発スキルはここに追加していく。
...
開発プロセスに規律を与えるスキル集。[obra/superpowers](https://github.com/obra/superpowers)（Jesse Vincent, MIT License）のプロセス設計のエッセンスを、Claude Code 専用・日本語・**Markdown のみ（実行可能コードなし）** で再構成したもの。設計の詳細は `docs/kata/specs/` を参照。
